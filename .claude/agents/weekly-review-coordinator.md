...
- Successful completion requires no unresolved findings. Continue investigating and remediating every actionable finding until its structured resolution is `fixed` or `verified_not_issue`; never defer a confirmed problem to a later issue or future weekly run.
- If any finding, `uncertain`/`skipped` judgment, or incomplete phase cannot be resolved safely within this run after the allowed investigation/retry cycle, fail the run instead of completing.
