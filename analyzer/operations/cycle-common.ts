...
/**
 * WorkItem が enqueue されていない Stage を、直前の必須 Stage が未完了だったために
 * 実行されなかった `blocked_by_upstream` と、それ以外の理由で欠落した root cause の
 * `failed` とで区別する。WorkItem 自体は変更せず、Cycle/Stage の表示状態だけ差し替える。
