...
// しきい値をより低く設定すると、お悔やみの返信や祝福の連投、ハッシュタグ企画への便乗といった、
// 自然な集団反応まで誤検知してしまうため、この値を採用している。
// しきい値の調整だけでは判定を完結できないため、低努力アカウントかどうかの追加判定と組み合わせる。
...
// 挨拶文化アカウントは元々リプライが多いため、低努力アカウントの追加判定でも区別できない。
// duplicate-reply-index.ts でも同じ理由から同じ値を採用している。
//
// なお、多数のアカウントがそれぞれ長文で実質的な返信を書く自然な集団反応は、
// 長さ・類似度のいずれでもこのルールの対象パターンと区別できないため、意図的に対象から外している。
