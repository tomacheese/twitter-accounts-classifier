...
// 「フォロバ」「相互フォロー」「無言フォロー」「DMください」のような通常のコミュニティ上の
// 礼儀表現は、ファンアカウントや懸賞アカウント、公式ブランドアカウントを含む多くの
// 正当な日本語アカウントで使われているため、あえて対象外とする。出会い系・裏垢DM販売・
// 自動フォロー宣言・副業勧誘など、明確に勧誘を示す文言のみを対象とする。
//
// 副業節の間隙は区切り記号・拒否記号を除外している。副業DMを拒否するbioは
// 「副業」の直後に拒否マーカーを置くため、無制限の間隙にするとその拒否をまたいで
// 勧誘と誤認してしまうため。
...
// SOLICITATION_PATTERN は用語の有無のみを判定し、bioがそれを拒否しているかは見ないため、
...
// followingCount が followersCount に対して高くなる。逆の形(フォロー少・フォロワー多)は
// 著名・公式アカウントの特徴であり、誤検知してはならない。
