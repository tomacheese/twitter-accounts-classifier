...
// 「ドラマ」だけは「ドラマチック」「ドラマー (drummer)」という無関係な頻出語の
// 部分文字列になるため、topic-nsfw.ts の「アダルト」除外と同様に
// 否定先読みで個別に除外している。
const MOVIE_PATTERN =
  /\b(movie|movies|film|cinema|moviegoer|cinephile)\b|映画|ドラマ(?!チック|ー)|洋画|邦画|映画館|映画鑑賞/i
