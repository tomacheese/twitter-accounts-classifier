...
/**
 * 課金による個人の X Blue 認証（青バッジ）を検出する。組織/政府認証を伴わない場合が対象。
 * この場合の `verifiedType` は `null`（未取得）または文字列 `'None'`（API が組織種別なしと報告）のいずれかを取り得るが、
 * どちらも「組織バッジなし」を意味する点は同じであり、
 * `verified-business`/`verified-government` と区別するために両方を許容する。
 */
