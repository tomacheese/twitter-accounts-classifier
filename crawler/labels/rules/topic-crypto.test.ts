...
describe('topicCryptoRule', () => {
  it('is true for a crypto/airdrop bio', () => {
    expect(
      topicCryptoRule.evaluate(makeBundle({ bio: '仮想通貨エアドロハンター #Airdrop #Crypto' })).value,
    ).toBe(true)
  })

  it('is true for an English crypto bio', () => {
    expect(topicCryptoRule.evaluate(makeBundle({ bio: 'NFT collector & web3 builder' })).value).toBe(true)
  })

  it('is false for an unrelated bio', () => {
    expect(topicCryptoRule.evaluate(makeBundle({ bio: '毎日ラーメンの写真を載せています' })).value).toBe(false)
  })
