...
// この「インプレゾンビ」的なアーキタイプ(言い換えや、時には AI による書き換え・翻訳を伴う
// リプライを1つのツイートに大量投稿する)は、隣接する他ルールでは捕捉できない。
// self_duplicate_reply は単独ツイートとリプライのペアを要求し(これはリプライ同士の比較)、
// templated_reply_network は異なるアカウント間での一字一句同一のテキストを要求する
// (これは同一アカウントによる言い換えテキストの繰り返し)ため。
...
export const replyFloodingRule: LabelRule = {
  ...
  version: '1.1.0',
  evaluate(bundle) {
    // 先頭の @メンション(会話相手)でグルーピングすると、通常の相互会話との区別が
    // できない。一人との往復チャットや口論でも、24時間以内に「同じ相手への」
    // 複数のリプライが容易に発生し、類似度の下限でも区別できないため。
    // 構造的な違いは「相手が誰か」ではなく「何に対するリプライか」にあり、
    // このアーキタイプは1つのバズったツイートに大量のリプライを積み上げるのに対し、
    // 通常の会話では毎回異なるツイートに返信する。本番データのリプライはすべて
    // `inReplyToTweetId` を持つため、これを基準にグルーピングしても失われる情報はなく、
