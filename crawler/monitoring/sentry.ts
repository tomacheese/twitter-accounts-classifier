...
// GLITCHTIP_DSN はローカル開発では未設定になり得るため、COOKIE_ISSUER_URL とは異なり、
// 未設定時に throw はせず意図的に no-op にしている。監視処理がクロール実行そのものを
// 止めてはならないため。
...
export function captureException(error: unknown, context?: Record<string, unknown>): void {
  ...
  // GlitchTip への報告はベストエフォートであり、ここでの throw を呼び出し元の
  // エラーハンドリングに伝播させてはならない。
  try {
...
/**
 * 例外以外のイベントを GlitchTip に報告する。captureException と同様、初期化前の
 * 呼び出しや送信失敗で throw すると呼び出し元の処理を止めてしまうため、
