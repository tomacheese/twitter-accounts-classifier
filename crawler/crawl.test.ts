...
describe('runCrawlCycle', () => {
  ...
  it('evaluates reply_hijack_swarm using the same effective index that screening used to admit the candidate', async () => {
    ...
    // persistAuthorResultAtomic に渡す replyHijackIndex もその 5 を反映するはずである。
    const callForCandidate = (
      deps.persistAuthorResultAtomic as ReturnType<typeof vi.fn>
    ).mock.calls.find(
      (call) => (call[0] as { authorId: string }).authorId === 'candidate3',
    )?.[0] as {
      replyHijackIndex: { swarmSizeFor: (accountId: string, tweetId: string) => number }
    }
  ...
  it('follow-graph 対象ラベルと author 単位で判明した accountId を loadFollowGraphLabelIndex に渡し、その結果を persistAuthorResultAtomic に渡す', async () => {
    ...
    const deps = makeDeps({
      ...
      ensureLabelDefinitions: vi.fn().mockResolvedValue(new Map([['topic_anime', 'ld1']])),
...
describe('runAuthorUnitPhase parent tweet fetch', () => {
  ...
  it('records a warning and continues when the parent tweet is not found in the response', async () => {
    const reply1 = rawTweet('reply1', rawUser('author1'), 'parent1')
    const getTweetDetail = vi.fn().mockResolvedValue({ data: { data: [] } })
    const findMissingTweetIds = vi.fn().mockResolvedValue(['parent1'])
    const persistAuthorResultAtomic = vi
    ...
    expect(metrics.warnings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ type: 'author_processing_failed', authorId: 'author1' }),
      ]),
    )
